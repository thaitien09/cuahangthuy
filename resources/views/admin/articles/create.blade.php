    ...
    <form action="{{ route('admin.articles.update', $article->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            ...
            <select class="form-control" id="status" name="status" required>
                <option value="nháp" {{ $article->status == 'nháp' ? 'selected' : '' }}>Nháp</option>
                <option value="xuất bản" {{ $article->status == 'xuất bản' ? 'selected' : '' }}>Xuất bản</option>
                <option value="lưu trữ" {{ $article->status == 'lưu trữ' ? 'selected' : '' }}>Lưu trữ</option>
            </select>
        </div>
